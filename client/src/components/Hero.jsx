...
const Hero = () => {
    ...
    const onSearch = async (e)=>{
            ...
            await axios.post("/api/user/store-recent-search", {recentSearchCities:destination},{headers:{Authorization:`Bearer ${await getToken()}`}})
            // add destination to searchedCities max 3 recent recent search cities
  ...
  return (
    <div className='flex flex-col items-start justify-center px-6 md:px-16 lg:px-24 xl:px-32 text-white bg-[url("/src/assets/heroImage.png")] bg-cover bg-no-repeat bg-center h-screen'>
      ...
      <p className='max-w-2xl mt-2 text-sm md:text-base'>Unparalleled luxury and comfort await at the world's most exclusive hotels and resorts. Start your journey today</p>
